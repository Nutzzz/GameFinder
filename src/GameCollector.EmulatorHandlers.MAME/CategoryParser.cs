...
using GameCollector.EmulatorHandlers.MAME.Properties;
...
public partial class CategoryParser
{
    ...
    public static string GetVersion(string exePath, string gameName, IFileSystem fileSystem)
    {
        if (!Versions.Any())
    ...
    public static Category GetCategory(string gameName, string exePath, IFileSystem fileSystem)
    {
        if (!Categories.Any())
            LoadCatFile(exePath, fileSystem);
    ...
    private static void LoadCatFile(string exePath, IFileSystem fileSystem)
    {
        var mamePath = fileSystem.Path.GetDirectoryName(exePath);
        if (mamePath is null)
            return;
        ...
        var catPathUser = @".\catver.ini";
        if (!fileSystem.File.Exists(catPathUser))
        {
            catPathUser = fileSystem.Path.Combine(mamePath, "catver.ini");
            if (!fileSystem.File.Exists(catPathUser))
                catPathUser = "";
        }
        if (!string.IsNullOrEmpty(catPathUser))
        {
            catText = fileSystem.File.ReadAllText(catPathUser);
            catVersionUser = GetCatVersion(catPathUser);
        }
